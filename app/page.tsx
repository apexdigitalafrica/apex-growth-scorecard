...
export default function Home() {
  ...
  useEffect(() => {
    fetch('/api/dashboard-stats?range=all')
      .then(res => res.json())
      .then(data => {
        setSubmissions(data.totalSubmissions || 127);
        setAverageScore(Math.round(data.averageScore) || 68);
        setCompanies((data.totalSubmissions || 89) + 50);
      })
      .catch(() => {
        // Fallback to animated counters
        let subCount = 0;
        let scoreCount = 0;
        let compCount = 0;
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-8 items-center opacity-60 hover:opacity-100 transition-opacity">
              {[1, 2, 3, 4, 5, 6].map((i) => (
                <div key={i} className="bg-white/5 backdrop-blur-md border border-white/10 rounded-xl p-6 h-24 flex items-center justify-center hover:scale-110 transition-transform">
                  <div className="text-white/40 text-xs text-center font-semibold">Company {i}</div>
                </div>
              ))}
            </div>
          </div>
        ...
        <div className="max-w-4xl mx-auto text-center px-4 sm:px-6 lg:px-8">
          <div className="bg-gradient-to-r from-red-500/10 to-orange-500/10 border border-red-400/30 rounded-3xl p-12 backdrop-blur-md">
            ...
            <p className="text-xl text-blue-200/80 mb-8">
              {submissions} businesses have already taken the assessment this month. 
            ...
            <div className="mt-6 text-blue-200/60 text-sm flex items-center justify-center gap-4">
              <span className="flex items-center gap-1"><Clock className="w-4 h-4" /> Takes only 5 minutes</span>
              <span className="flex items-center gap-1"><CheckCircle className="w-4 h-4" /> Completely free</span>
              <span className="flex items-center gap-1"><Lock className="w-4 h-4" /> Your data is secure</span>
            </div>
          </div>
        </div>
      </div>
