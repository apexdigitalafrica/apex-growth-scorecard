...
export function computeFunnelScore(snapshot: FunnelSnapshot): FunnelScoreSummary {
  const stageScores: StageScore[] = snapshot.stages.map((stage) => {
    ...
    const targetBoost =
      stage.targetConversion != null
        ? clamp((conversionRate / stage.targetConversion) * 100, 0, 120) * 0.15
        : 0;
  ...
  return {
    ...
    avgConversionRate,
    biggestLeakStage: sortedByDropOff[0],
    strongestStage: sortedByScore[0],
    stageScores,
  };
}
...
export function generateFunnelInsights(
  ...
  if (summary.overallScore >= 80) {
    ...
    insights.push({
      ...
      severity: 'medium',
      actionItems: [
        'Map customer journey at weakest stage',
  ...
  if (summary.biggestLeakStage) {
    ...
    insights.push({
      ...
      severity: 'high',
      actionItems: [
        'Record customer calls to find objections',
        'Simplify this stage by 50%',
        'Add social proof',
  ...
  if (summary.strongestStage) {
    insights.push({
      ...
      severity: 'medium',
      actionItems: [
        'Document what works here',
        'Interview successful customers',
        'Apply to weaker stages',
  ...
  if (impact?.estimatedExtraRevenue) {
    ...
    insights.push({
      ...
      severity: 'high',
      actionItems: [
        'Calculate ROI of fixes vs new traffic',
        'Focus 70% on fixes, 30% on traffic',
        'Set 30-day sprint for top 3 leaks',
        'Measure daily',
      ],
