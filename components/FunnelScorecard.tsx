...
const handleExportVisualPDF = async () => {
  ...
  try {
    ...
    const { generateAndDownloadPDF } = await import('@/components/FunnelReportPDF');
    await generateAndDownloadPDF(snapshot);
  } catch (error) {
    console.error('PDF export failed:', error);
    alert('PDF export failed. Please try again or use the text report.');
  } finally {
    setIsGeneratingPDF(false);
  }
};

// In your JSX, update the button to:
<button
  onClick={handleExportVisualPDF}
  disabled={isGeneratingPDF}
  className={`group flex items-center gap-2 bg-gradient-to-r from-emerald-600 to-cyan-600 hover:from-emerald-500 hover:to-cyan-500 text-white font-bold px-5 py-2.5 rounded-xl shadow-lg shadow-emerald-500/30 transition-all ${
    isGeneratingPDF 
...
>
  ...
  {isGeneratingPDF ? 'Generating...' : 'Visual PDF'}
</button>
